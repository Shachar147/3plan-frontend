    ...
    return (
        <div className={getClasses("trip-template-page-cover", isShimmering && 'shimmer-animation')} style={{
            ...
            <div className={`${baseClass}-content-bottom-shadow`}>
                <span className={getClasses(isShimmering && 'shimmer-animation')} style={isShimmering ? {
                    width: "100%",
                    minHeight: 40
                }:{}}>{getEventTitle(c, eventStore, true)}</span>
                {isInTemplate && isShimmering ? <div className="shimmer-animation border-radius-20" style={{
                        width: 128,
                        height: 38
                    }} /> :(
                    <Button
