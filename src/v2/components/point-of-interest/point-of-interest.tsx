...
const Image = ({
	...
	const fallbacks = [
		'/images/trip-photo-1.jpg',
		'/images/trip-photo-2.png',
		'/images/trip-photo-3.png',
		'/images/trip-photo-4.png',
		'/images/trip-photo-5.png',
		'/images/trip-photo-2.png',
	];
	...
	return (
		<>
			<div
				className={getClasses('shimmer-animation', className)}
				style={{
					height: isSmall ? 200 : 266,
					width: isSmall ? 298 : 400,
				}}
			/>
			<img
				src={src}
				alt={alt}
				onError={() => setSrc(fallbacks[random])}
				onLoad={(e) => {
				...
				className={getClasses('display-none zoomable', className)}
				key={idx}
			/>
			<div
				className={getClasses(className, !backgroundImage && 'display-none')}
				style={{
				...
				<div className="carousel-wrapper">
					<div className="carousel-root">
						<div className="carousel carousel-slider width-100-percents">
							...
							/>
							<p className="carousel-status">1 of 1</p>
						</div>
					</div>
				</div>
			</div>
				...
				<div className={getClasses('category-label', eventStore.isMobile && 'is-mobile')}>
					<div className={getClasses(isSmall && 'padding-bottom-4')}>
						<div className="flex-row gap-8 flex-wrap-wrap align-items-center">
							...
							/>
							<span className="shimmer-animation" style={{ width: 100, height: 22, borderRadius: 10 }} />
						</div>
					</div>
...
const PointOfInterest = ({
	...
	isEditMode,
	onEditSave,
	onEditDescriptionSave,
	onEditCategorySave,
	onEditDestinationsSave,
	isSmall,
	suggestionsMode,
	...
	const category = extractCategory([item.name ?? '', item.description ?? ''].filter(Boolean));
	...
	const handleAddToPlan = () => {
		...
		if (item.category) {
			...
			if (existingCategory.length == 0) {
				...
				existingCategory = eventStore.categories.filter(
					(c) =>
						c.title === title ||
						c.title === TranslateService.translate(eventStore, title) ||
						c.title ==
							TranslateService.translateFromTo(
								eventStore,
								title,
								{},
								'en',
						...
						c.title ==
							TranslateService.translateFromTo(
								eventStore,
								`CATEGORY.${title.toUpperCase()}`.replace('CATEGORY.CATEGORY.', 'CATEGORY.'),
								{},
								'en',
								eventStore.calendarLocalCode
							) ||
		...
		ReactModalService.openAddSidebarEventModal(
			...
			categoryId,
			{
				...
				priority: !item.priority
					? item.isSystemRecommendation
						? TriplanPriority.must
						: item.rate?.rating && item.rate?.rating == 5
	...
	const alreadyInPlan = !![...eventStore.calendarEvents, ...eventStore.allSidebarEvents].find(
		(i) => i.extra?.feedId == feedId
	);
		...
		return (
			<>
				<Button
					...
					text={
						alreadyInPlan
							? TranslateService.translate(eventStore, 'POINT_OF_INTEREST.ADDED_TO_PLAN')
							: TranslateService.translate(eventStore, 'POINT_OF_INTEREST.ADD_TO_PLAN')
					}
	...
	function renderCategoryName() {
			...
			return (
				<div className="flex-row gap-3">
					...
					<span>
						{(item.destination ?? '')
							.split(',')
							.map((d) => TranslateService.translate(eventStore, d.trim()))
							.join(', ')}
					</span>
				</div>
			);
		...
		return <span onClick={onLabelClick}>{name}</span>;
	}
		...
		if (isEditMode) {
			...
			return (
				<div className="flex-row gap-10 justify-content-center align-items-center">
					<DestinationSelector
						hideSelectedChips
						onChange={(newDestinations) => onEditDestinationsSave?.(newDestinations)}
	...
	function renderName() {
		...
		const name = (
			<EditableLabel
				...
				placeholder={TranslateService.translate(eventStore, 'name')}
	...
	return (
		<div
			className={getClasses(
				'point-of-interest',
				isHebrew && 'hebrew-mode',
				(mainFeed || isSmall) && 'main-feed',
				savedCollection && 'saved-collection',
				myTrips && 'my-trips-poi',
		...
		>
			...
			<div className="poi-right">
				{(item.priority === 'high' || item.isSystemRecommendation) && (
					<div className="top-pick-label">{TranslateService.translate(eventStore, 'TOP_PICK')}</div>
				)}
				{item.category && !mainFeed && renderItemCategory()}
				<div className={getClasses('name-container', isEditMode && 'margin-bottom-10')}>{renderName()}</div>
				<span className={getClasses('description', isShrinkedMode && !isViewItem && 'max-height-100-ellipsis')}>
					{onEditDescriptionSave ? (
						<EditableLabel
							...
							placeholder={TranslateService.translate(eventStore, 'TEMPLATE.DESCRIPTION')}
							isEditMode={isEditMode}
							onEditSave={onEditDescriptionSave}
				...
				<div className="poi-details">
					...
					{!savedCollection && !myTrips && (item.price ?? item.extra?.price) && (
						<span className="price">
							{TranslateService.translate(eventStore, 'POINT_OF_INTEREST.PRICE', {
								price: item.price ?? item.extra?.price,
								currency: getCurrencySign(item),
							})}
						</span>
					)}
					{!!item.rate && !!item.rate.rating && (
						<div className="rate">
							{renderStars(item.rate.rating)}
							<span>
								{TranslateService.translate(
									eventStore,
									item.rate.quantity && item.source !== 'System'
										? 'POINT_OF_INTEREST.REVIEWS'
										: 'POINT_OF_INTEREST.REVIEWS.SHORT',
									{
										rate: rating,
										rateMax: 5,
						...
						<div className="poi-footer-links">
							...
							{item.location && (
								<a
									...
									rel="noopener noreferrer"
								>
									{TranslateService.translate(eventStore, 'VIEW_ON_GOOGLE_MAPS')}
								</a>
							)}
							<div className="source-logo">
								<img
									src={`/images/${item.source?.toLowerCase()?.replaceAll('.', '')}.png`}
									alt={item.source}
								/>
							</div>
						</div>
