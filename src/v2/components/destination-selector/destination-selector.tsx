...
function DestinationSelector(props: DestinationSelectorProps) {
	...
	useEffect(() => {
		if (props.selectedDestinations) {
			if (
				allOptions.length > 0 &&
				JSON.stringify(prevSelectedDestinations.current) != JSON.stringify(props.selectedDestinations)
			) {
			...
			<Select
				...
				formatOptionLabel={(option: OptionType) => (
					<div className="option-label">
						...
						{option.isPopular && (
							<i
								className="fa fa-star triplan-orange-color"
								title={TranslateService.translate(
									eventStore,
									'DESTINATION_SELECTOR.POPULAR_DESTINATION'
								)}
							/>
						)}
					</div>
