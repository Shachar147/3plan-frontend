...
const FeedView = ({ eventStore, mainFeed, searchKeyword, viewItemId, filterByDestination, suggestionsMode, withHideSuggestionsButton }: FeedViewProps) => {
    ...
    function renderShowingResultsText() {
        ...
        return (
            <div className={getClasses("flex-1-1-0 min-width-max-content gap-4", eventStore.getCurrentDirection() === 'rtl' && 'direction-rtl')}>
                ...
                {/*})}</span>*/}
                {isFiltering && <span className="width-100-percents text-align-end">{ TranslateService.translate(eventStore, 'SHOWING_X_FROM_Y', {
    ...
    return (
        (feedStore.isLoading && !haveNoDestinations) ? renderLoadingPlaceholder() : <LazyLoadComponent className="feed-view bright-scrollbar" disableLoader={(mainFeed && !eventStore.isMobile) || viewItemId} fetchData={(page, setLoading) => fetchItems(page, setLoading)} isLoading={feedStore.isLoading} isReachedEnd={feedStore.allReachedEnd}>
