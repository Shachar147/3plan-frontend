...
function TriplanHeaderLine({
	...
	const signInOutBtn = (
		<Button
			...
			text={
				isLoggedIn
					? eventStore.isMobile
						? TranslateService.translate(eventStore, 'LOGOUT')
						: `${TranslateService.translate(eventStore, 'LOGOUT')}, ${getUser()}`
					: `${TranslateService.translate(eventStore, 'LOGIN')}`
			}
			className={
				window.location.href.includes('/login') && !window.location.hash.includes('register') && 'active'
			}
			onClick={() => {
	...
	return (
		<>
			...
			{condition3 && !condition1 && !condition2 && <div className="mobile-search">{search}</div>}
		</>
	);
}
