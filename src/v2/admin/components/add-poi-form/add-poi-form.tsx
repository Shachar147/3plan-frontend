...
} from '../../../components/destination-selector/destination-selector';
...
function POIForm() {
	...
	const fields = [
		{
			name: 'location',
			label: TranslateService.translate(eventStore, 'ADMIN_MANAGE_ITEM.LOCATION'),
			type: 'location-selector',
			isRequired: true,
		},
		{
			name: 'more_info',
			label: TranslateService.translate(eventStore, 'SOURCE_OR_LINK'),
			type: 'text',
			isLink: true,
			placeholderKey: 'LINKS_ONLY',
			isRequired: true,
		},
		{ name: 'name', label: TranslateService.translate(eventStore, 'EVENT_NAME'), type: 'text', isRequired: true },
		{
			name: 'destination',
			label: TranslateService.translate(eventStore, 'ADMIN_MANAGE_ITEM.DESTINATION'),
			type: 'destination-selector',
			isRequired: true,
		},
		{
			name: 'duration',
			label: TranslateService.translate(eventStore, 'MODALS.DURATION'),
			type: 'text',
			isRequired: true,
		},
		{
			name: 'images',
			label: TranslateService.translate(eventStore, 'MODALS.IMAGES'),
			type: 'image-upload',
			isRequired: true,
		},
		{
			name: 'description',
			label: TranslateService.translate(eventStore, 'ADMIN_MANAGE_ITEM.DESCRIPTION'),
			type: 'textarea',
			isRequired: true,
		},
		{
			name: 'category',
			label: TranslateService.translate(eventStore, 'TEMPLATE.CATEGORY'),
		...
		{
			name: 'currency',
			label: TranslateService.translate(eventStore, 'MODALS.CURRENCY'),
	...
	const handleImageProcessing = async () => {
		...
		const promises = formData.images.map((file, index) => {
			...
			return fileUploadService
				.uploadPhoto(file, `/images/pois/${localPrefix}${sanitizedPOIName}-${index + 1}.${extension}`)
				.then((photo) => {
	...
	const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
		...
		if (name == 'price' || name.includes('rate')) {
			if (value == undefined || Number(value) <= e.target.getAttribute('min')) {
				// <= since min defined as -1
				value = undefined;
		...
		if (name == 'location') {
			if (!value?.latitude || !value?.longitude) {
				...
				if (!formData.more_info) {
					...
					const foundCategory =
						eventStore.modalValues['category']?.label &&
					...
					if (
						...
						formData.category.includes(TranslateService.translate(eventStore, 'CATEGORY.HOTELS'))
					) {
	...
	function validateBeforeSubmit() {
		...
		for (let i = 0; i < fields.length; i++) {
			...
			const hasValues =
				field.name == 'location' ? !!formData[field.name] : (formData[field.name]?.length ?? 0) > 0;
		...
		if (formData['duration'].split(':').length != 2 || !getDurationInMs(formData['duration'])) {
			ReactModalService.internal.alertMessage(eventStore, 'MODALS.ERROR.TITLE', 'DURATION_IS_INVALID', 'error');
			isOk = false;
		}
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		try {
			...
			if (response.totalUpdated) {
				...
				ReactModalService.internal.alertMessage(eventStore, 'MODALS.ERROR.TITLE', 'FAILED_TO_ADD_POI', 'error');
			}
		} catch (error) {
	...
	function renderInput({
			...
			return (
				<div className="flex-column gap-4">
					...
					<div className="image-upload-buttons">
						<label className={getClasses('file-label', isSaving && 'disabled')} htmlFor="file-upload">
							{TranslateService.translate(
								eventStore,
								previewUrls.length > 0 ? 'CHANGE_FILES' : 'CHOOSE_A_FILE'
							)}
						</label>
						{previewUrls.length > 0 && (
							<label className="file-label remove" onClick={() => handleImageRemoval(undefined)}>
								{TranslateService.translate(eventStore, 'REMOVE_FILES')}
							</label>
						)}
						{previewUrls.length > 0 && (
							<label className="file-label remove" onClick={() => handleImageRemoval(currentIdx.current)}>
								{TranslateService.translate(eventStore, 'REMOVE_FILE')}
							</label>
						)}
					</div>
					<input
		...
		if (type == 'currency-selector') {
			return ReactModalRenderHelper.renderCurrencySelector(
				eventStore,
				'currency',
				{
					name,
		...
		if (type == 'destination-selector') {
			return (
				<DestinationSelector
					isDisabled={isSaving}
					selectedDestinations={formData.destination ? [formData.destination] : undefined}
					isSingle
					onChange={(destinations: string[]) => {
						handleChange({
							target: {
								name,
			...
			return (
				<LocationInput
					...
					placeholder={TranslateService.translate(
						eventStore,
						placeholderKey ?? 'MODALS.LOCATION.PLACEHOLDER'
					)}
	...
	function renderLabel(label: string, isRequired?: boolean) {
		return (
			<label>
				{label}:{isRequired && <span className="red-color margin-inline-start-3">*</span>}
			</label>
		);
	}
	...
	function getSavingText() {
		...
		if (isUploadingPictures) {
			return TranslateService.translate(eventStore, 'UPLOADING_PHOTOS_X_OF_Y', {
	...
	return (
		<div className="add-poi-form-container">
			<div className="add-poi-form">
				...
				<Button
					isLoading={isSaving}
					tooltip={isSaving ? TranslateService.translate(eventStore, 'PLEASE_WAIT_WHILE_SAVING') : undefined}
