					...
					if (orderedEvents.length > 0) {
						const descriptionKey =
							orderedEvents.length == 1
								? 'CLEAR_CALENDAR_EVENTS.LOCKED_ITEM.DESCRIPTION'
								: 'CLEAR_CALENDAR_EVENTS.LOCKED_ITEMS.DESCRIPTION';

						const content = () => (
							<div className="flex-col align-items-center justify-content-center gap-10">
								<div>
									{TranslateService.translate(eventStore, descriptionKey, {
						...
						ReactModalService.openConfirmModalContent(
							eventStore,
							eventStore.clearNonOrderedCalendarEvents.bind(eventStore),
							undefined,
							content,
							'CLEAR_ONLY_NON_ORDERED'
						);
					} else {
											...
											return (
												...
												>
													<TriplanSidebarDraggableEvent
														...
														eventTitleSuffix={toDistanceString(
															eventStore,
															x,
															true,
															x.travelMode,
	...
	const renderCalendarSidebarStatistics = () => {
		...
		const renderPriceList = () => {
			...
			function getContent(priceList: Record<TriplanCurrency, number>, titleKey: string) {
				...
				const bookedCalendarEventsPerDay: Record<string, EventInput> =
					ListViewService._buildCalendarEventsPerDay(
						eventStore,
				...
				const calendarTotalPricePerDay: Record<string, MinMax> = ListViewService._getEstimatedCosts(
					eventStore,
					calendarEventsPerDay,
					desiredCurrency,
					false
				);
				const bookedTotalPricePerDay: Record<string, MinMax> = ListViewService._getEstimatedCosts(
					eventStore,
				...
				let bookedTotal = (
					Object.values(bookedTotalPricePerDay).reduce(
						(prev, iter) => prev + (iter as MinMax).max,
						0
					) as number
				).toFixed(2);
				let calendarMinTotal = (
					Object.values(calendarTotalPricePerDay).reduce(
						(prev, iter) => prev + (iter as MinMax).min,
						0
					) as number
				).toFixed(2);
				let calendarMaxTotal = (
					Object.values(calendarTotalPricePerDay).reduce(
						(prev, iter) => prev + (iter as MinMax).max,
						0
					) as number
				...
				const nonCalendarTotalPricePerDay: Record<string, MinMax> = ListViewService._getEstimatedCosts(
					eventStore,
					{
				...
				let nonCalendarMaxTotal = (
					Object.values(nonCalendarTotalPricePerDay).reduce(
						(prev, iter) => prev + (iter as MinMax).max,
						0
					) as number
				).toFixed(2);
				let nonCalendarMinTotal = (
					Object.values(nonCalendarTotalPricePerDay).reduce(
						(prev, iter) => prev + (iter as MinMax).min,
						0
					) as number
				...
				if (titleKey == 'ESTIMATED_PRICE_OF_SCHEDULED_ACTIVITIES') {
					...
					priceList = {
						// @ts-ignore
						[desiredCurrency]:
							calendarMinTotal == calendarMaxTotal
								? calendarMaxTotal
								: `${calendarMinTotal} - ${calendarMaxTotal}`,
					};
					...
					priceList = {
						// @ts-ignore
						[desiredCurrency]:
							nonCalendarMinTotal == nonCalendarMaxTotal
								? nonCalendarMaxTotal
								: `${nonCalendarMinTotal} - ${nonCalendarMaxTotal}`,
					};
				...
				const shouldShowPriceInline =
					(isUnknown ||
						!(
							priceListWithStringValues[Object.keys(priceListWithStringValues)[0] as TriplanCurrency] ||
							''
						).includes('-')) &&
					!eventStore.isMobile;
...
const FilterBy = observer(() => {
	const [isOpen, setIsOpen] = useState(false);
	const eventStore = useContext(eventStoreContext);
	const arrowDirection = eventStore.getCurrentDirection() === 'ltr' ? 'right' : 'left';

	function renderPriorityFilters() {
		const priorityFiltersContent = (
			<div className="flex-column gap-8">
				{prioritiesOrder.map((priority, idx) => {
					const priorityKey = TriplanPriority[priority];
					const priorityColor = priorityToColor[priority];

					return (
						<Observer key={`sidebar-filter-by-priority-${priorityKey}`}>
							{() => (
								<div
									className={getClasses(
								...
								>
									<i className="fa fa-sticky-note" style={{ color: priorityColor }} />
									<span>{TranslateService.translate(eventStore, priorityKey.toString())}</span>
								</div>
							)}
						</Observer>
					);
					...
					return (
						<Observer key={`sidebar-filter-by-preferred-time-${timeKey}`}>
							{() => (
								...
								>
									{icon}
									<span>{TranslateService.translate(eventStore, timeKey.toLowerCase())}</span>
								</div>
							)}
						</Observer>
					);
		...
		<div className="external-collapse">
			<div className="sidebar-statistics sidebar-group" onClick={() => setIsOpen(!isOpen)}>
				...
				<span>
					...
					&nbsp;
					{TranslateService.translate(eventStore, 'FILTER_BY')}
				</span>
			</div>
			<div
				className="external-collapse-inner"
