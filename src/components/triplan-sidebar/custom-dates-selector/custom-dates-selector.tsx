...
import DataServices, {DateRangeFormatted} from "../../../services/data-handler-interfaces";
    ...
    return (
        <div className={getClasses(["custom-dates-container"], !eventStore.isCalendarView && 'hidden')}>
            <div className={"custom-dates-line"}>
                <input type={"date"} key={`date-range-selector-${customDateRange.start}`} value={customDateRange.start} onChange={(e) => {
                ...
                <input type={"date"} key={`date-range-selector-${customDateRange.end}`} value={customDateRange.end} onChange={(e) => {
                    const value = e.target.value;
                    const newCustomDateRange = {
