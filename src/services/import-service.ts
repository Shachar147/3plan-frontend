						...
						if (val === '') {
							...
							if (eventStore.allEvents.find((e) => e.title === val)) {
								...
								const error = `${TranslateService.translate(
									eventStore,
									'EVENT_WITH_NAME'
								)} ${val} ${TranslateService.translate(eventStore, 'ALREADY_EXISTS')}.`;
								console.error(error);
								errors.push(error);
								numOfEventsWithErrors[i] = 1;
							}
